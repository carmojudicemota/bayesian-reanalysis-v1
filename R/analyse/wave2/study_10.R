load_study_10_wave2_data <- function(
    ...
    path = paste0(
      "data/raw/study_10/",
      "master.anonymizedOSF.csv"
    )) {

  if (!file.exists(path)) {
    stop(
      "Study 10 data file does not exist: ",
      path,
      call. = FALSE
    )
  }
  ...
  if (nrow(data) == 0L) {
    stop(
      "Study 10 has no complete observations ",
      "for ",
      outcome_column,
      ".",
      call. = FALSE
...
compute_study_10_bayes_factors <- function(claim,priors) {
  outcome_column <-
    study_10_outcome(claim$claim_id)
  data <-
    load_study_10_wave2_data(outcome_column =outcome_column)
